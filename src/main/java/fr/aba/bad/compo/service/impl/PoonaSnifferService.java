...
public class PoonaSnifferService implements RankingProviderService, PlayerInfoProviderService {
	...
	private static final String RANKS_RE = Arrays.stream(Rank.values()).map(Enum::name).collect(Collectors.joining("|"));
	private static final Pattern RANKING_PATTERN = Pattern.compile(".*Classement\\s*:\\s*("+RANKS_RE+").*", Pattern.MULTILINE | Pattern.DOTALL);
